...
### Shippable 
Et nytt krav for denne releasen var at prosjektet skulle ha konfigurasjonen for å lage et shippable produkt. Måten vi gjorde dette var å sette inn en jpackage-maven-plugin i fxui sin pom. Deretter måtte de som har windows laste ned WiX. For å generere shippable produktet skriver man inn kommandoen: mvn clean compile javafx:jlink jpackage:jpackage. Dette genererer en dist mappe i fxui sin target som inneholder bankappfx-1.0.0.exe. Man trykker på denne filen for å laste ned BankAppFX appen. For å kjøre programmen lokalt fra pcen må man kjøre springboot i bakgrunnen. Om springboot kjører i bakgrunenn kan man direkte trykke på BankAppFX for å kjøre appen.
